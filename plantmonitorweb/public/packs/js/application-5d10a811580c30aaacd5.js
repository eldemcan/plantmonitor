...
var Hello = /*#__PURE__*/function (_Controller) {
  _inherits(Hello, _Controller);
  ...
  function Hello() {
    ...
    _this.ranges = {
      minute: _this.range(1, 59),
      hour: _this.range(1, 23),
      days: _this.range(1, 31),
      month: _this.rangeForMonth(),
      daysw: _this.rangeForDaysOfWeek()
    };
    return _this;
  }
  ...
  _createClass(Hello, [{
    ...
    value: function connect() {
      console.log('hello wfrom fecking stimulus');
    }
  }, {
    key: "blockTime",
    value: function blockTime(e) {
    ...
    value: function sendData(e) {
      var jobsDropdown = this.jobsTarget;
      console.log(jobsDropdown[jobsDropdown.selectedIndex].value);
      console.log(this.jobsTarget);
    ...
    key: "rangeForDaysOfWeek",
    value: function rangeForDaysOfWeek() {
      return ['Sun', 'Mon', 'Tue', 'Wed', 'Thur', 'Fri', 'Sat'].map(function (item, index) {
        return {
          value: index,
          displayValue: item
        };
    ...
    key: "rangeForMonth",
    value: function rangeForMonth() {
      return ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'].map(function (item, index) {
        return {
          value: index,
          displayValue: item
        };
    ...
    key: "greet",
    value: function greet(e) {
      console.log(e);
      var selectedIndex = e.target.selectedIndex;
      var selectedValue = e.target[selectedIndex].value;
      var selectBox = this.numsTarget;
      selectBox.innerHTML = '';
      this.ranges[selectedValue].forEach(function (item) {
    ...
    key: "reloadJobs",
    value: function reloadJobs(e) {
      var _this2 = this;

      var submitButton = this.createJobTarget;
      submitButton.disabled = true;
      setTimeout(fetch("/scheduled_jobs", {
        method: "GET"
      }).then(function (response) {
        return response.json();
      }).then(function (data) {
        console.log('dadsadata is ', data);
    ...
    key: "addRow",
    value: function addRow(items) {
      var jobsTable = this.statusTarget;
      jobsTable.innerHTML = ''; // var tableRows = jobsTable.getElementsByTagName('tr')
      // var rowCount = tableRows.length;
      //  for (var x=rowCount-1; x>0; x--) {
      //    console.log(tableRows[x])
      //     jobsTable.removeChild(tableRows[x])
      //  }
      // while(jobsTable.hasChildNodes()) {
      // jobsTable.removeChild(jobsTable.firstChild)
      // }
      // for (let index = 1; index < jobsTable.rows.length; index++) {
      //   console.log(index)
      //   jobsTable.deleteRow(-1)
      // }

      items.forEach(function (jobs, index) {
        console.log('going to add', jobs);
        var row = jobsTable.insertRow();
        var cell1 = row.insertCell(0);
        var cell2 = row.insertCell(1);
        var cell3 = row.insertCell(2);
        cell1.innerHTML = "<th scope=row> ".concat(index, "  </th>");
        cell2.innerHTML = "<th> ".concat(jobs.name, " </th>");
        cell3.innerHTML = "<th> ".concat(jobs.cron, " </th>");
      });
    }
  }]);
...
Hello.targets = ["status", "nums", "jobs", "createJob", "time"];
Hello.baseUrl = '';
...
/******/ });
//# sourceMappingURL=application-5d10a811580c30aaacd5.js.map